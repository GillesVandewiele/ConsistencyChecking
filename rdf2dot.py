...
def rdf2dot(g, stream, opts={}):
    ...
    for s, p, o in g:
        ...
        if isinstance(o, (rdflib.URIRef, rdflib.BNode)):
            on = node(o)
            opstr = u"\t%s -> %s [ color=%s, label=%s ] ;\n"
            stream.write(opstr % (sn, on, color(p), re.sub(':', '_', qname(p, g))))
        else:
            fields[sn].add((qname(p, g), formatliteral(o, g)))

    for u, n in nodes.items():
        stream.write(u"# %s %s\n" % (u.encode('ascii', 'ignore').decode('ascii'), n.encode('ascii', 'ignore').decode('ascii')))
        f = [u"<tr><td align='left'>%s</td><td align='left'>%s</td></tr>" %
             x for x in sorted(fields[n])]

        opstr = u"%s [ shape=none, color=%s label=%s ]"#< <table color='#666666'" + \
        ...
        _label = pattern.sub('', label(u.encode('ascii', 'ignore').decode('ascii'), g))
        if _label != "":
            stream.write(opstr % (n, NODECOLOR, _label))
        else:
            stream.write(opstr % (n, NODECOLOR, 'empty'))
                                                  #, 
                                                  #u.encode('ascii', 'ignore').decode('ascii'), u.encode('ascii', 'ignore').decode('ascii'), 
                                                  #u"".join(f).encode('ascii', 'ignore').decode('ascii')))
